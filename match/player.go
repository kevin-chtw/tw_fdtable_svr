...
func (p *Player) HandleMessage(ctx context.Context, req *cproto.MatchReq) *cproto.MatchAck {
	...
	ack.Ack = &cproto.MatchAck_SignupAck{
		SignupAck: &cproto.SignupAck{ErrorCode: 400},
	}
	return ack
}
...
func (p *Player) createSignupAck(matchid string, errorCode int32) *cproto.MatchAck {
	...
	// 添加调试信息
	if errorCode != 0 {
		p.logWithContext(context.Background(), "DEBUG", "createSignupAck error", "matchid", matchid, "errorCode", errorCode)
	}
	return ack
}

func (p *Player) createEnterMatchAck(matchid, tableid string, players []string) *cproto.MatchAck {
	return &cproto.MatchAck{
